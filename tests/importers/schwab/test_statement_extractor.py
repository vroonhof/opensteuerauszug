...
class TestStatementExtractorPositions(unittest.TestCase):
    ...
    def _create_mock_extractor(self, extracted_data):
        ...
        mock_extractor._next_business_day = StatementExtractor._next_business_day.__get__(
            mock_extractor, StatementExtractor
        )

        # Need to bind the method to the mock instance for it to work correctly within extract_positions
        mock_extractor.extract_positions = StatementExtractor.extract_positions.__get__(
            mock_extractor, StatementExtractor
        )
    ...
    def test_extract_positions_uses_opening_and_closing_values(self):
        ...
        self.assertEqual(
            close_date_plus1, date(2023, 2, 1)
        )  # 31st Jan is Tue, next biz day is 1st Feb
        self.assertEqual(depot, 'AWARDS')
        self.assertEqual(
            len(positions),
            4,
            "Should have 2 security (open/close) and 2 cash (open/close) positions",
        )
        ...
        for pos_obj, stock_obj in positions:
            if isinstance(pos_obj, SecurityPosition):
                ...
                if stock_obj.referenceDate == date(2023, 1, 1):
                    self.assertEqual(
                        stock_obj.quantity, decimal.Decimal('10'), "Opening shares mismatch"
                    )
                    found_security_open = True
                elif stock_obj.referenceDate == date(2023, 2, 1):
                    self.assertEqual(
                        stock_obj.quantity, decimal.Decimal('15'), "Closing shares mismatch"
                    )
            ...
            if isinstance(pos_obj, CashPosition):
                ...
                if stock_obj.referenceDate == date(2023, 1, 1):
                    self.assertEqual(
                        ...
                        "Opening cash quantity mismatch",
                    )
                    self.assertEqual(
                        stock_obj.balance,
                        decimal.Decimal('1000.00'),
                        "Opening cash balance mismatch",
                    )
                    ...
                    self.assertEqual(
                        ...
                        "Closing cash quantity mismatch",
                    )
                    self.assertEqual(
                        stock_obj.balance,
                        decimal.Decimal('1500.00'),
                        "Closing cash balance mismatch",
                    )
    ...
    def test_extract_positions_handles_missing_opening_values(self):
        ...
        self.assertEqual(
            ...
            2,
            "Should only have closing security and closing cash positions",
        )
        ...
        self.assertFalse(
            has_opening_security_position,
            "No opening security position should be created if opening_shares is None",
        )
        ...
        self.assertFalse(
            has_opening_cash_position,
            "No opening cash position should be created if opening_cash is None",
        )

        # Check that closing positions are still created correctly
        has_closing_security_position = any(
            isinstance(p, SecurityPosition)
            and s.referenceDate == expected_close_date_plus1
            and s.quantity == decimal.Decimal('20')
            for p, s in positions_missing
        )
        self.assertTrue(
            has_closing_security_position, "Closing security position not found or incorrect"
        )

        has_closing_cash_position = any(
            isinstance(p, CashPosition)
            and s.referenceDate == expected_close_date_plus1
            and s.quantity == decimal.Decimal('2000.00')
            for p, s in positions_missing
        )
