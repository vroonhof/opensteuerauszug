...
@pytest.mark.parametrize(
    "managed_temp_csv_path", ["Depot,Date,Quantity\nACC123,2023-01-01,100\n"], indirect=True
)
...
def test_extract_positions_valid_csv_awards_cash(managed_temp_csv_path):
    ...
    assert pos.depot == "AWARDS"
    assert pos.currentCy == "USD"
    assert pos.cash_account_id == "GOOG"
...
def test_extract_positions_optional_currency_column(managed_temp_csv_path):
    ...
    cash_pos, cash_stock = results[0]
    assert isinstance(cash_pos, CashPosition)
    assert cash_pos.currentCy == "EUR"
    assert cash_stock.balanceCurrency == "EUR"

    sec_pos, sec_stock = results[1]
    assert isinstance(sec_pos, SecurityPosition)
    assert sec_stock.balanceCurrency == "USD"
...
def test_extract_positions_multiple_valid_entries(managed_temp_csv_path):
    ...
    assert pos2.cash_account_id is None
...
@pytest.mark.parametrize(
    "managed_temp_csv_path",
    ["Depot,Date,Symbol,Quantity\n123,2023-01-15,CASH 123,1000.00\n"],
    indirect=True,
)
def test_extract_positions_legacy_cash_with_suffix_rejected(managed_temp_csv_path):
    extractor = FallbackPositionExtractor(managed_temp_csv_path)
    log_capture = StringIO()
    with contextlib.redirect_stdout(log_capture):
        results = extractor.extract_positions()
    ...
    assert results is None
    output = log_capture.getvalue()
    assert "Symbol 'CASH 123'" in output
    assert "legacy CASH format" in output


@pytest.mark.parametrize(
    "managed_temp_csv_path",
    ["Depot,Date,Symbol,Quantity\nAWARDS,2023-01-15,AAPL,100\n"],
    indirect=True,
)
def test_extract_positions_legacy_awards_depot_rejected(managed_temp_csv_path):
    extractor = FallbackPositionExtractor(managed_temp_csv_path)
    log_capture = StringIO()
    with contextlib.redirect_stdout(log_capture):
        results = extractor.extract_positions()
