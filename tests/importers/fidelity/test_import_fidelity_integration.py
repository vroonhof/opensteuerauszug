...
def test_fidelity_import_integration(sample_dir):
    ...
    has_securities = (
        tax_statement.listOfSecurities is not None
        and tax_statement.listOfSecurities.depot
        and len(tax_statement.listOfSecurities.depot) > 0
    )

    has_bank_accounts = (
        tax_statement.listOfBankAccounts is not None
        and tax_statement.listOfBankAccounts.bankAccount
        and len(tax_statement.listOfBankAccounts.bankAccount) > 0
    )

    assert (
        has_securities or has_bank_accounts
    ), f"Sample in {sample_dir} imported no securities and no bank accounts"
