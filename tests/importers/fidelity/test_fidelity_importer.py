...
from opensteuerauszug.importers.fidelity.fidelity_importer import (
    FidelityImporter,
)
...
def test_invariant_missing_account_detail_entry_is_not_skipped():
    assert not should_skip_pseudo_account_entry(
        DummyFidelityEntry(account_id=None, level_of_detail="DETAIL")
    )

# Define a basic sample FIDELITY Flex Query XML as a string
SAMPLE_FIDELITY_INPUT_TRADES_VALID = """
...
SAMPLE_FIDELITY_INPUT_AGGREGATE = """
...
SAMPLE_FIDELITY_INPUT_MISSING_FIELD = """
...
SAMPLE_FIDELITY_INTEREST_INTEREST_WITH_SECURITY = """
...
SAMPLE_FIDELITY_FLEX_XML_TRANSFER = """
...
SAMPLE_FIDELITY_FLEX_XML_TRANSFER_WRONG_SIGN = """
...
SAMPLE_FIDELITY_FLEX_XML_TRANSFER_CANCELLED_OUT = """
...
SAMPLE_FIDELITY_FLEX_XML_TRANSFER_ONLY_WITH_OPEN_POSITION = """
...
SAMPLE_FIDELITY_FLEX_XML_OPEN_POSITION_ONLY = """
...
SAMPLE_FIDELITY_FLEX_XML_STOCK_SPLIT = """
<FlexQueryResponse queryName="SplitQuery" type="AF">
  <FlexStatements count="1">
    <FlexStatement accountId="U1234567" fromDate="2025-01-01" toDate="2025-12-31" period="Year" whenGenerated="2026-01-15T10:00:00">
      <OpenPositions>
        <OpenPosition accountId="U1234567" assetCategory="STK" symbol="FIDELITY" description="INTERACTIVE BROKERS GRO-CL A" conid="43645865" isin="US45841N1072" currency="USD" position="8" markPrice="64.31" positionValue="514.48" reportDate="2025-12-31" />
      </OpenPositions>
      <CorporateActions>
        <CorporateAction accountId="U1234567" assetCategory="STK" symbol="FIDELITY" description="INTERACTIVE BROKERS GRO-CL A" conid="43645865" isin="US45841N1072" currency="USD" reportDate="2025-06-18" dateTime="2025-06-17;202500" actionDescription="FIDELITY(US45841N1072) SPLIT 4 FOR 1 (FIDELITY, INTERACTIVE BROKERS GRO-CL A, US45841N1072)" quantity="6" type="FS" />
      </CorporateActions>
      <CashReport>
        <CashReportCurrency accountId="U1234567" currency="USD" endingCash="0" fromDate="2025-01-01" toDate="2025-12-31" />
      </CashReport>
...
SAMPLE_FIDELITY_FLEX_XML_CORPORATE_ACTION_EXCHANGE = """
...
SAMPLE_FIDELITY_FLEX_XML_WITH_MISSING_ACCOUNT_ENTRIES = """
...
SAMPLE_FIDELITY_FLEX_XML_WITH_HYPHEN_ACCOUNT_ENTRIES = """
...
@pytest.fixture
def sample_fidelity_settings() -> List[FidelityAccountSettings]:
    return [
        FidelityAccountSettings(
            account_number="U1234567",
            broker_name="Interactive Brokers",
            account_name_alias="Test FIDELITY Account",
...
@pytest.fixture
def sample_fidelity_settings_other_account() -> List[FidelityAccountSettings]:
    return [
        FidelityAccountSettings(
            account_number="U7654321",
            broker_name="Interactive Brokers",
            account_name_alias="Test FIDELITY Account Missing",
            canton="ZH",  # Placeholder
...
def test_fidelity_import_valid_xml(sample_fidelity_settings):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)

    importer = FidelityImporter(
        period_from=period_from, period_to=period_to, account_settings_list=sample_fidelity_settings
    )

    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_INPUT_VALID)
...
def test_fidelity_import_keeps_entries_with_missing_account_id(sample_fidelity_settings):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_WITH_MISSING_ACCOUNT_ENTRIES)
        xml_file_path = tmp_file.name
...
def test_fidelity_import_skips_hyphen_account_entries(sample_fidelity_settings):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_WITH_HYPHEN_ACCOUNT_ENTRIES)
        xml_file_path = tmp_file.name
...
def test_security_payment_quantity_is_minus_one(sample_fidelity_settings):
...
def test_withholding_tax_reversal_nets_against_withholding(sample_fidelity_settings):
...
def test_transfer_to_stock(sample_fidelity_settings):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
    ...
    importer = FidelityImporter(
        ...
        period_to=period_to,
        account_settings_list=sample_fidelity_settings,
    )

    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_TRANSFER)
        xml_file_path = tmp_file.name
...
def test_transfer_quantity_sign_mismatch(sample_fidelity_settings):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_TRANSFER_WRONG_SIGN)
        xml_file_path = tmp_file.name
...
def test_transfer_with_open_position_no_trades(sample_fidelity_settings):
    """
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_TRANSFER_ONLY_WITH_OPEN_POSITION)
        xml_file_path = tmp_file.name
...
def test_open_position_only_no_mutations(sample_fidelity_settings):
    """
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_OPEN_POSITION_ONLY)
        xml_file_path = tmp_file.name
...
def test_fidelity_trade_aggregation(sample_fidelity_settings):
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_AGGREGATE)
        xml_file_path = tmp_file.name
...
def test_fidelity_import_missing_required_field(sample_fidelity_settings_other_account):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
    ...
    importer = FidelityImporter(
        period_from=period_from,
        period_to=period_to,
        account_settings_list=sample_fidelity_settings_other_account,
    )

    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_MISSING_FIELD)
        xml_file_path = tmp_file.name
...
def test_open_position_balance_date_is_day_after_period_end(sample_fidelity_settings):
...
def test_cash_transaction_security_interest_assert(sample_fidelity_settings):
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_INTEREST_WITH_SECURITY)
        xml_file_path = tmp_file.name
...
# - FX rates if applicable (though Fidelity Flex often provides data in base currency or has fxRateToBase)
...
def test_import_files_with_client_information(client_data, sample_fidelity_settings):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
...
def test_import_files_with_firstname_and_name(monkeypatch, sample_fidelity_settings):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
...
def test_base_summary_currency_filtered_out(sample_fidelity_settings):
...
def test_corporate_action_stock_split_creates_mutation(sample_fidelity_settings):
    period_from = date(2025, 1, 1)
    period_to = date(2025, 12, 31)
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_STOCK_SPLIT)
        xml_file_path = tmp_file.name
    ...
    try:
        ...
        depot = tax_statement.listOfSecurities.depot[0]
        fidelity_sec = next(
            (s for s in depot.security if s.securityName == "Fidelity"),
            None,
        )
        assert fidelity_sec is not None

        split_mutation = next(
            (s for s in fidelity_sec.stock if s.mutation and s.referenceDate == date(2025, 6, 18)),
            None,
        )
        ...
        opening_balance = next(
            (s for s in fidelity_sec.stock if not s.mutation and s.referenceDate == period_from),
            None,
        )
...
def test_corporate_action_exchange_creates_mutations(sample_fidelity_settings):
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_CORPORATE_ACTION_EXCHANGE)
        xml_file_path = tmp_file.name
...
def test_bank_account_names_always_set(sample_fidelity_settings):
...
def test_fidelity_import_canton_extraction(sample_fidelity_settings):
    """Test that canton is extracted from Fidelity stateResidentialAddress."""
    period_from = date(2023, 1, 1)
    period_to = date(2023, 12, 31)
...
def test_fidelity_import_canton_extraction_different_cantons(sample_fidelity_settings):
...
def test_fidelity_import_canton_extraction_invalid_format(sample_fidelity_settings):
...
def test_fidelity_import_canton_extraction_invalid_swiss_canton(sample_fidelity_settings):
...
def test_fidelity_import_account_dates_set_on_bank_accounts(sample_fidelity_settings):
...
def test_fidelity_import_account_dates_both_set(sample_fidelity_settings):
...
def test_fidelity_import_account_dates_scoped_to_own_statement(sample_fidelity_settings):
...
def test_fidelity_import_account_dates_not_set_when_absent(sample_fidelity_settings):
...
def test_fidelity_import_canton_extraction_no_account_info(sample_fidelity_settings):
...
# ibflex.enable_unknown_attribute_tolerance() before importing Fidelity data so
...
def test_strict_mode_rejects_unknown_xml_attributes(sample_fidelity_settings):
...
def test_unknown_xml_attributes_are_silently_ignored(sample_fidelity_settings):
    """With tolerance enabled, unknown attributes must not break parsing.

    The production driver enables this mode before importing Fidelity data so
...
@pytest.mark.usefixtures("_enable_unknown_attribute_tolerance")
def test_unknown_xml_element_types_are_silently_ignored(sample_fidelity_settings):
...
def test_withholding_tax_cash_transactions_are_mapped_to_security_tax_fields(sample_fidelity_settings):
    period_from = date(2025, 1, 1)
    period_to = date(2025, 12, 31)
...
# Anonymized from real Fidelity Flex Query export (Yield Enhancement Income program).
...
#   listingExchange="" ... amount="0.32" type="Withholding Tax" ...
#   levelOfDetail="DETAIL" ... />
SAMPLE_FIDELITY_FLEX_XML_WHTAX_NO_SECURITY = """
...
def test_withholding_tax_without_security_creates_bank_payment(sample_fidelity_settings):
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_WHTAX_NO_SECURITY)
        xml_file_path = tmp_file.name
...
def test_trade_unit_price_set_when_not_zero(sample_fidelity_settings):
...
def test_trade_unit_price_none_when_zero(sample_fidelity_settings):
...
def test_trade_unit_price_various_nonzero_and_zero_prices(sample_fidelity_settings):
...
def test_cancelled_out_transfer_is_allowed_and_labelled(sample_fidelity_settings):
    """
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_TRANSFER_CANCELLED_OUT)
        xml_file_path = tmp_file.name
...
SAMPLE_FIDELITY_FLEX_XML_SHORT_POSITION = """
...
def test_short_stock_position_negative_balance_raises(sample_fidelity_settings):
    """
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_SHORT_POSITION)
        xml_file_path = tmp_file.name
...
SAMPLE_FIDELITY_FLEX_XML_OPT_C_SPREAD = """
...
SAMPLE_FIDELITY_FLEX_XML_OPT_P_OPEN = """
...
def test_opt_c_spread_closes_intra_year(sample_fidelity_settings):
    """
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_OPT_C_SPREAD)
        xml_file_path = tmp_file.name
...
def test_opt_p_open_at_year_end(sample_fidelity_settings):
    """
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_OPT_P_OPEN)
        xml_file_path = tmp_file.name
...
SAMPLE_FIDELITY_FLEX_XML_ADR_FEE = """
...
def test_security_linked_adr_fee_is_ignored(sample_fidelity_settings):
    """
    ...
    See issue: "Payment reconciliation: Fidelity ADR Fee shown as mismatch"
    """
    period_from = date(2025, 1, 1)
    period_to = date(2025, 12, 31)
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_ADR_FEE)
        xml_file_path = tmp_file.name
...
SAMPLE_FIDELITY_FLEX_XML_PAYMENT_IN_LIEU = """
<FlexQueryResponse queryName="TestQuery" type="AF">
  <FlexStatements count="1">
    <FlexStatement accountId="U1234567" fromDate="2025-01-01" toDate="2025-12-31" period="Year" whenGenerated="2026-01-15T10:00:00">
      <OpenPositions>
...
def test_payment_in_lieu_of_dividends_is_marked_as_securities_lending(sample_fidelity_settings):
    """
    ...
    See issue: "FIDELITYLITYLITY: Position with Payment In Lieu Of Dividends payments deduction
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as tmp_file:
        tmp_file.write(SAMPLE_FIDELITY_FLEX_XML_PAYMENT_IN_LIEU)
        xml_file_path = tmp_file.name
...
SAMPLE_FIDELITY_FLEX_XML_BND_MAIN = """
...
SAMPLE_FIDELITY_FLEX_XML_BND_CORRECTIONS = """
...
def test_fidelity_corrections_flex_imports_withholding_reversals(sample_fidelity_settings):
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as f_main:
        f_main.write(SAMPLE_FIDELITY_FLEX_XML_BND_MAIN)
        main_path = f_main.name
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".xml") as f_corr:
        f_corr.write(SAMPLE_FIDELITY_FLEX_XML_BND_CORRECTIONS)
...
def test_fidelity_corrections_flex_skips_out_of_period_transactions(sample_fidelity_settings):
