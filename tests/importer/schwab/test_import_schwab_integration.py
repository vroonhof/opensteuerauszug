...
from src.opensteuerauszug.importers.schwab.SchwabImporter import SchwabImporter
...
def test_schwab_import_integration(sample_dir):
    ...
    importer = SchwabImporter(period_from=period_from, period_to=period_to)
    tax_statement = importer.import_dir(sample_dir)
    assert isinstance(tax_statement, TaxStatement)
