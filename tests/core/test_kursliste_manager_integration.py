...
def test_load_directory_xml_only(tmp_path):
    ...
    assert isinstance(
        accessor_2023.data_source, List
    ), "Expected List[Kursliste] for XML-only scenario"
...
def test_load_directory_sqlite_only(tmp_path):
    ...
    assert isinstance(
        accessor_2024.data_source, KurslisteDBReader
    ), "Expected KurslisteDBReader for SQLite-only scenario"
...
def test_load_directory_xml_and_sqlite_preference(tmp_path):
    ...
    assert isinstance(
        accessor_2025.data_source, KurslisteDBReader
    ), "Expected KurslisteDBReader when both XML and SQLite exist (SQLite preference)"
...
def test_load_directory_multiple_years_mixed(tmp_path):
    ...
    assert isinstance(accessor_2026.data_source, List) and isinstance(
        accessor_2026.data_source[0], Kursliste
    ), "Type mismatch for 2026 (XML only)"
    ...
    assert isinstance(
        accessor_2027.data_source, KurslisteDBReader
    ), "Type mismatch for 2027 (SQLite only)"
    ...
    assert isinstance(
        accessor_2028.data_source, KurslisteDBReader
    ), "Type mismatch for 2028 (XML and SQLite)"
...
def test_load_directory_non_kursliste_files_ignored(tmp_path):
    ...
    (tmp_path / "not_kursliste_2029.xml").write_text(
        "<data></data>"
    )  # Invalid name pattern for year extraction
...
def test_load_directory_specific_sqlite_name_preference(tmp_path):
    ...
    temp_xml_specific_content = temp_xml_specific.read_text().replace(
        "Test Share AG 2036", "Specific Share AG 2036"
    )
    ...
    assert security is not None
    assert (
        security.securityName == "Specific Share AG 2036"
    )  # Content from the specifically named DB
