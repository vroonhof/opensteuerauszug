...
class TestPositionReconciler(unittest.TestCase):
    ...
    def test_multiple_mutations(self):
        ...
        try:
            ...
            self.fail(
                "check_consistency raised ValueError unexpectedly for multi-mutation consistent data"
            )
    ...
    def test_synthesize_position_at_later_balance(self):
        ...
        if pos_after_final_balance is not None:
            ...
            self.assertEqual(
                pos_after_final_balance.currency, "GBP"
            )  # Currency from the 2023-01-15 balance
    ...
    def test_out_of_order_event_in_consistency_check(self):
        ...
        reconciler.sorted_stocks.append(
            create_stock("2022-12-31", "10", True)
        )  # Event before current_date in check
        ...
        stocks_for_test = [
            ...
            create_stock(
                "2023-01-03", "10", True, name="MutationOK"
            ),  # current_date becomes 2023-01-03
        ...
        reconciler_direct.sorted_stocks.append(
            create_stock("2023-01-02", "5", True, name="LateOutOfOrderMutation")
        )
        # Resort to place it in processing order, but with wrong date logic if safeguard fails
        reconciler_direct.sorted_stocks = sorted(
            reconciler_direct.sorted_stocks, key=lambda s: (s.referenceDate, s.mutation)
        )
        ...
        reconciler_forced.sorted_stocks = (
            forced_stocks  # Manually set the unsorted (by date logic) list
        )
    ...
    def test_synthesize_backward_with_intervening_mutations(self):
        stocks = [
            ...
            create_stock(
                "2023-02-01", "220", False, currency="USD", name="Future Balance"
            ),  # Future Bal = 220. After M2: 220-30=190. After M1: 190-(-10)=200
        ]
    ...
    def test_synthesize_backward_chooses_earliest_future_balance(self):
        stocks = [
            ...
            create_stock(
                "2023-01-15", "-5", True, name="MutationBetween"
            ),  # Should be unapplied from 220 -> 225
