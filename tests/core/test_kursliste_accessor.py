...
<kursliste xmlns="http://xmlns.estv.admin.ch/ictax/2.0.0/kursliste"
           ...
           version="2.0.0.0" creationDate="{datetime.now().isoformat()}" year="{TAX_YEAR}">
    ...
    <share id="1" quoted="true" source="KURSLISTE" securityGroup="SHARE" securityType="SHARE.COMMON" 
        ...
        <yearend id="10" quotationType="PIECE" taxValue="150.50" taxValueCHF="150.50" />
    </share>
    <bond id="2" quoted="true" source="KURSLISTE" securityGroup="BOND" securityType="BOND.BOND" 
          ...
          institutionId="2002" institutionName="Inst Acc B1" issueDate="{TAX_YEAR-2}-01-01" redemptionDate="{TAX_YEAR+8}-01-01">
        <yearend id="20" quotationType="PERCENT" percent="101.25" taxValue="1012.50" />
    </bond>
    <fund id="3" quoted="true" source="KURSLISTE" securityGroup="FUND" securityType="FUND.DISTRIBUTION" 
          valorNumber="1112223" isin="LU00000000F1" securityName="Accessor Fund DB MultiValor1" 
          currency="USD" nominalValue="1.0" country="LU" 
          institutionId="2003" institutionName="Inst Acc F1">
    </fund>
    <fund id="4" quoted="true" source="KURSLISTE" securityGroup="FUND" securityType="FUND.ACCUMULATION" 
          valorNumber="1112223" isin="LU00000000F2" securityName="Accessor Fund DB MultiValor2" 
          currency="USD" nominalValue="1.0" country="LU" 
          institutionId="2003" institutionName="Inst Acc F1">
    </fund>
     <share id="5" quoted="true" source="KURSLISTE" securityGroup="SHARE" securityType="SHARE.COMMON" 
           valorNumber="2223334" isin="CH00000000S1" securityName="Accessor Share DB MultiISIN" 
           currency="CHF" nominalValue="10.00" country="CH" 
           institutionId="2001" institutionName="Inst Acc S1">
    </share>
    <share id="6" quoted="true" source="KURSLISTE" securityGroup="SHARE" securityType="SHARE.COMMON"
           valorNumber="3334445" isin="CH00000000S7" securityName="Old Accessor Share DB"
           currency="CHF" nominalValue="5.0" country="CH"
           institutionId="2005" institutionName="Inst Acc Old"> 
    </share>
...
def db_reader_fixture(tmp_path):
    ...
    cmd = [
        "python", str(CONVERSION_SCRIPT_PATH_ACCESSOR),
        str(sample_xml_file),
        str(output_db_file)
    ]
...
def test_get_security_by_valor_singular_db(db_accessor):
    ...
    assert db_accessor.get_security_by_valor(3334445) is None 
...
def test_get_securities_by_valor_plural_db(db_accessor):
    ...
    assert names == ["Accessor Fund DB MultiValor1", "Accessor Fund DB MultiValor2"]
    ids = sorted([s.id for s in secs])
    assert ids == [3, 4] 
    for sec in secs:
        assert sec.valorNumber == 1112223
        assert sec.institutionId == 2003
        assert sec.institutionName == "Inst Acc F1"

    secs_single = db_accessor.get_securities_by_valor(7654321) 
    assert len(secs_single) == 1
    assert isinstance(secs_single[0], Bond)
    assert secs_single[0].id == 2

    assert len(db_accessor.get_securities_by_valor(8888888)) == 0

def test_get_security_by_isin_singular_db(db_accessor):
    sec = db_accessor.get_security_by_isin("CH00000000B1") 
    assert sec is not None
    assert isinstance(sec, Bond)
    assert sec.id == 2
    assert sec.securityName == "Accessor Bond DB"
    assert sec.valorNumber == 7654321
    assert sec.institutionId == 2002
    assert sec.institutionName == "Inst Acc B1"
...
def test_get_securities_by_isin_plural_db(db_accessor):
    ...
    secs_single = db_accessor.get_security_by_isin("LU00000000F1") 
    assert len(secs_single) == 1
    assert secs_single[0].id == 3
