...
CONVERSION_SCRIPT_PATH_ACCESSOR = (
    PROJECT_ROOT_ACCESSOR / "scripts" / "convert_kursliste_to_sqlite.py"
)
...
def db_reader_fixture(tmp_path):
    ...
    assert (
        output_db_file.exists()
    ), "SQLite DB file was not created by conversion script for accessor test."
...
def xml_kursliste_list_fixture():
    kl = Kursliste(
        ...
        shares=[
            Share(
                id=1,
                securityName="Share XML AG",
                isin="CH0123456789",
                valorNumber=900100,
                currency="CHF",
                nominalValue=Decimal("20.0"),
                country="CH",
                securityGroup=SecurityGroupESTV.SHARE,
                securityType=SecurityTypeESTV.SHARE_COMMON,
                institutionId=1,
                institutionName="Inst XML",
            ),
            Share(
                id=3,
                securityName="MultiValor XML 1",
                isin="US0123456789",
                valorNumber=900300,
                currency="USD",
                nominalValue=Decimal("60.0"),
                country="US",
                securityGroup=SecurityGroupESTV.SHARE,
                securityType=SecurityTypeESTV.SHARE_COMMON,
                institutionId=1,
            ...
            Share(
                id=4,
                securityName="MultiValor XML 2",
                isin="US0123456780",
                valorNumber=900300,
                currency="USD",
                nominalValue=Decimal("65.0"),
                country="US",
                securityGroup=SecurityGroupESTV.SHARE,
                securityType=SecurityTypeESTV.SHARE_PREFERRED,
                institutionId=1,
                institutionName="Inst XML",
            ),
        ],
        bonds=[
            Bond(
                id=2,
                securityName="Bond XML SA",
                isin="FR0123456789",
                valorNumber=900200,
                currency="EUR",
                nominalValue=Decimal("500.0"),
                country="FR",
                securityGroup=SecurityGroupESTV.BOND,
                securityType=SecurityTypeESTV.BOND_BOND,
                institutionId=2,
                institutionName="Inst XML 2",
                issueDate=date(2020, 1, 1),
                redemptionDate=date(2030, 1, 1),
            ),
        ],
        funds=[
            Fund(
                id=5,
                securityName="MultiISIN XML Fund 1",
                isin="GB0123456789",
                valorNumber=900400,
                currency="GBP",
                nominalValue=Decimal("1.0"),
                country="GB",
                securityGroup=SecurityGroupESTV.FUND,
                securityType=SecurityTypeESTV.FUND_DISTRIBUTION,
                institutionId=3,
                institutionName="Inst XML 3",
            ),
            Fund(
                id=6,
                securityName="MultiISIN XML Fund 2",
                isin="GB0123456789",
                valorNumber=900500,
                currency="GBP",
                nominalValue=Decimal("1.0"),
                country="GB",
                securityGroup=SecurityGroupESTV.FUND,
                securityType=SecurityTypeESTV.FUND_ACCUMULATION,
                institutionId=3,
                institutionName="Inst XML 3",
            ),
        ],
        exchangeRates=[
            ExchangeRate(currency="USD", date=date(TAX_YEAR, 10, 21), value=Decimal("0.89"))
        ],
        exchangeRatesMonthly=[
            ExchangeRateMonthly(currency="EUR", year=TAX_YEAR, month="11", value=Decimal("0.96"))
        ],
        exchangeRatesYearEnd=[
            ExchangeRateYearEnd(currency="GBP", year=TAX_YEAR, value=Decimal("1.16"))
        ],
    )
    return [kl]
