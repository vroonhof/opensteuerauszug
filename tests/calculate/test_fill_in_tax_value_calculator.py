...
class TestFillInTaxValueCalculatorIntegration:
    ...
    def test_run_in_verify_mode_no_errors(self, sample_file: str, exchange_rate_provider: KurslisteExchangeRateProvider):
        ...
        """
        calculator = FillInTaxValueCalculator(mode=CalculationMode.VERIFY, exchange_rate_provider=exchange_rate_provider)
        ...
        if filtered_errors:
            ...
            error_details = "\n".join(error_messages)
            pytest.fail(f"FillInTaxValueCalculator produced errors for {sample_file} with {len(filtered_errors)} errors:\n{error_details}")
