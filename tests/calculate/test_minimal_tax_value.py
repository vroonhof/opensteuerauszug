...
def minimal_tax_value_calculator_overwrite() -> MinimalTaxValueCalculator:
    ...
    return MinimalTaxValueCalculator(
...
def test_minimal_tax_value_calculator_calculate_empty_statement_verify(
    ...
    assert (
        len(minimal_tax_value_calculator_verify.modified_fields) == 0
    )  # Verify mode should not modify
...
class TestMinimalTaxValueCalculatorIntegration:
    ...
    def test_run_in_verify_mode_no_errors(
        ...
        calculator = MinimalTaxValueCalculator(
        ...
        def _is_expected_payment_error(error: Exception) -> bool:
            ...
            if (
                ".payment" in error.field_path
                and error.expected is None
                and isinstance(error.actual, SecurityPayment)
            ):
                return True
            return False
        ...
        filtered_errors = [
            e
            for e in calculator.errors
            if not _known_issue(e, tax_statement_input.institution)
            and not _is_expected_payment_error(e)
        ]
        ...
        if filtered_errors:
            ...
            error_details = "\n".join(error_messages)
            pytest.fail(
                f"MinimalTaxValueCalculator produced errors for {sample_file} with {len(filtered_errors)} errors:\n{error_details}"
            )
...
class TestMinimalTaxValueCalculatorHandlers:
    def test_handle_bank_account_ch(
    ...
    def test_handle_bank_account_foreign(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
    ...
    def test_handle_bank_account_unknown(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
    ...
    def test_handle_bank_account_tax_value_conversion(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        batv = BankAccountTaxValue(
            balance=Decimal("1000"), balanceCurrency="USD", referenceDate=date(2023, 12, 31)
        )
    ...
    def test_handle_bank_account_tax_value_chf(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        batv = BankAccountTaxValue(
            balance=Decimal("1000"), balanceCurrency="CHF", referenceDate=date(2023, 12, 31)
        )
        calculator._handle_BankAccountTaxValue(batv, "batv")
    ...
    def test_handle_bank_account_tax_value_uses_year_end_rate(self, db_exchange_rate_provider):
        calculator = MinimalTaxValueCalculator(
            mode=CalculationMode.FILL, exchange_rate_provider=db_exchange_rate_provider
        )
        batv = BankAccountTaxValue(
            balance=Decimal("1000"), balanceCurrency="USD", referenceDate=date(TAX_YEAR, 12, 31)
        )
        calculator._handle_BankAccountTaxValue(batv, "batv")
        assert batv.exchangeRate == Decimal("0.8800")
        assert batv.value == Decimal("880.0")

    def test_handle_bank_account_payment_type_a(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        calculator._current_account_is_type_A = True
        bap = BankAccountPayment(
            amount=Decimal("200"), amountCurrency="CHF", paymentDate=date(2023, 6, 30)
        )
        ...
        assert not bap.grossRevenueB or bap.grossRevenueB == Decimal(
            "0"
    ...
    def test_handle_bank_account_payment_type_b(
        ...
        bap = BankAccountPayment(
            amount=Decimal("200"), amountCurrency="USD", paymentDate=date(2023, 6, 30)
        )
        calculator._handle_BankAccountPayment(bap, "bap")
        ...
        assert bap.grossRevenueA is None or bap.grossRevenueA == Decimal(
            "0"
        )  # Type A not applicable
        assert bap.withHoldingTaxClaim is None or bap.withHoldingTaxClaim == Decimal(
            "0"
        )  # Type B not applicable

    def test_handle_bank_account_payment_type_unknown_error(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        calculator._current_account_is_type_A = None
        bap = BankAccountPayment(
            amount=Decimal("100"), amountCurrency="USD", paymentDate=date(2023, 1, 1)
        )
        with pytest.raises(ValueError) as excinfo:
            calculator._handle_BankAccountPayment(bap, "bap")
        assert "parent BankAccount has no country specified" in str(excinfo.value)

    def test_handle_bank_account_payment_zero_revenue(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        calculator._current_account_is_type_A = True
        bap = BankAccountPayment(
            amount=Decimal("0"), amountCurrency="CHF", paymentDate=date(2023, 6, 30)
        )
        calculator._handle_BankAccountPayment(bap, "bap")
    ...
    def test_handle_liability_account_tax_value(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        latv = LiabilityAccountTaxValue(
            balance=Decimal("5000"), balanceCurrency="EUR", referenceDate=date(2023, 12, 31)
        )
        calculator._handle_LiabilityAccountTaxValue(latv, "latv")
        assert latv.exchangeRate == Decimal("0.5")
        assert latv.value == Decimal("2500")

    def test_handle_liability_account_payment(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        lap = LiabilityAccountPayment(
            amount=Decimal("100"), amountCurrency="GBP", paymentDate=date(2023, 5, 5)
        )
    ...
    def test_handle_security_foreign(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        sec = Security(
    ...
    def test_handle_security_tax_value_conversion(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
    ...
    def test_handle_security_payment_no_op(
        self, minimal_tax_value_calculator_fill: MinimalTaxValueCalculator
    ):
        calculator = minimal_tax_value_calculator_fill
        sp = SecurityPayment(
...
def test_broker_payments_are_snapshotted_before_kursliste_overwrite(
    minimal_tax_value_calculator_overwrite: MinimalTaxValueCalculator,
):
...
def test_existing_broker_payments_are_not_replaced_when_present(
    minimal_tax_value_calculator_overwrite: MinimalTaxValueCalculator,
):
    calculator = minimal_tax_value_calculator_overwrite
