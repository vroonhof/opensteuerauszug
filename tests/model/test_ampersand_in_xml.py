...
def test_ampersand_in_security_name_xml_serialization():
    ...
    assert (
        security_name_attr == "ISHARES CORE S&P 500"
    ), "Security name should be unescaped in the element attribute"
    ...
    assert (
        'securityName="ISHARES CORE S&amp;P 500"' in xml_string
    ), "XML should have escaped ampersand"
    # Make sure we don't have an unescaped ampersand in the attribute
    assert (
        'securityName="ISHARES CORE S&P 500"' not in xml_string
    ), "XML should not have unescaped ampersand"
