...
def compute_org_nr(tax_statement: TaxStatement, override_org_nr: Optional[str] = None) -> str:
    ...
    if override_org_nr is not None:
        ...
        if (
            not isinstance(override_org_nr, str)
            or not override_org_nr.isdigit()
            or len(override_org_nr) != 5
        ):
            raise ValueError(
                f"Invalid org_nr format: '{override_org_nr}'. Must be a 5-digit string."
            )
