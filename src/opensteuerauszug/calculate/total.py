...
class TotalCalculator(BaseCalculator):
    ...
    def __init__(
        self, mode: CalculationMode = CalculationMode.FILL, round_sub_total: bool = True
    ) -> None:
        super().__init__(mode)
    ...
    def _process_tax_statement(self, tax_statement: TaxStatement) -> None:
        ...
        if tax_statement.listOfSecurities:
                ...
                for i, depot in enumerate(tax_statement.listOfSecurities.depot):
                    if depot.security:
                        for j, security in enumerate(depot.security):
                            ...
                            if security.payment:
                                for payment in security.payment:
                                    ...
                                    if payment.additionalWithHoldingTaxUSA is not None:
                                        sec_additional_withholding_tax_usa += (
                                ...
                                if self.mode in [CalculationMode.FILL, CalculationMode.OVERWRITE]:
                                    ...
                                    security.totalWithHoldingTaxClaim = round_accounting(
                                        sec_withholding
                                    )
                                    security.totalNonRecoverableTax = round_accounting(
                                        sec_non_recoverable_tax
                                    )
                                    security.totalAdditionalWithHoldingTaxUSA = round_accounting(
                                        sec_additional_withholding_tax_usa
                                    )
                            ...
                            list_additional_withholding_tax_usa += (
                                sec_additional_withholding_tax_usa
                            )
            ...
            self._round_and_set_field(
                tax_statement.listOfSecurities, 'totalTaxValue', list_tax_value, "listOfSecurities"
            )
            self._round_and_set_field(
                tax_statement.listOfSecurities,
                'totalGrossRevenueA',
                list_revenue_a,
                "listOfSecurities",
            )
            ...
            self._round_and_set_field(
                tax_statement.listOfSecurities,
                'totalLumpSumTaxCredit',
                list_lump_sum_tax_credit,
                "listOfSecurities",
            )
            self._round_and_set_field(
                tax_statement.listOfSecurities,
            ...
            self._round_and_set_field(
                ...
                list_non_recoverable_tax,
                "listOfSecurities",
            )
            self._round_and_set_field(
                tax_statement.listOfSecurities,
                'totalGrossRevenueIUP',
                Decimal('0'),
                "listOfSecurities",
            )
            self._round_and_set_field(
                tax_statement.listOfSecurities,
                'totalGrossRevenueConversion',
                Decimal('0'),
                "listOfSecurities",
            )
        ...
        if tax_statement.listOfBankAccounts and tax_statement.listOfBankAccounts.bankAccount:
            ...
            for i, account in enumerate(tax_statement.listOfBankAccounts.bankAccount):
                ...
                if account.payment:
                    for payment in account.payment:
                        if payment.grossRevenueA is not None:
                            acc_revenue_a += payment.grossRevenueA
                        if payment.grossRevenueB is not None:
                            acc_revenue_b += payment.grossRevenueB
                        if payment.withHoldingTaxClaim is not None:
                            acc_withholding += payment.withHoldingTaxClaim
                ...
                if self.mode in [CalculationMode.FILL, CalculationMode.OVERWRITE]:
                    ...
                    self._set_field_value(
                        account, 'totalGrossRevenueA', rounded_acc_revenue_a, path
                    )
                    self._set_field_value(
                        account, 'totalGrossRevenueB', rounded_acc_revenue_b, path
                    )
                    self._set_field_value(
                        account, 'totalWithHoldingTaxClaim', rounded_acc_withholding, path
                    )
            ...
            self._round_and_set_field(
                tax_statement.listOfBankAccounts,
                'totalTaxValue',
                list_tax_value,
                "listOfBankAccounts",
            )
        ...
        if tax_statement.listOfLiabilities and tax_statement.listOfLiabilities.liabilityAccount:
            ...
            for i, account in enumerate(tax_statement.listOfLiabilities.liabilityAccount):
                ...
                if account.payment:
                    for payment in account.payment:
                        if payment.grossRevenueB is not None:
                            liab_revenue_b += payment.grossRevenueB
            ...
            self._round_and_set_field(
                tax_statement.listOfLiabilities,
                'totalTaxValue',
                list_tax_value,
                "listOfLiabilities",
            )
        ...
        self._round_and_set_field(
            tax_statement, 'totalGrossRevenueA', self.total_gross_revenue_a, ""
        )
        self._round_and_set_field(
            tax_statement, 'totalGrossRevenueB', self.total_gross_revenue_b, ""
        )
        self._round_and_set_field(
            tax_statement, 'totalWithHoldingTaxClaim', self.total_withholding_tax_claim, ""
        )
        ...
        if self.mode in [CalculationMode.FILL, CalculationMode.OVERWRITE]:
            ...
            tax_statement.summaryGrossRevenueA = round_accounting(
                self.total_gross_revenue_a_summary
            )
            tax_statement.summaryGrossRevenueB = round_accounting(
                self.total_gross_revenue_b_summary
            )
            tax_statement.steuerwert_ab = (
                tax_statement.summaryTaxValueA + tax_statement.summaryTaxValueB
            )
