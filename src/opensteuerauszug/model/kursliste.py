...
class SecurityTypeESTV(str, Enum):
    BOND_BOND = "BOND.BOND"
    BOND_CONVERTIBLE = "BOND.CONVERTIBLE"
    BOND_OPTION = "BOND.OPTION"
    COINBULL_COINGOLD = "COINBULL.COINGOLD"
    COINBULL_GOLD = "COINBULL.GOLD"
    COINBULL_PALLADIUM = "COINBULL.PALLADIUM"
    COINBULL_PLATINUM = "COINBULL.PLATINUM"
    COINBULL_SILVER = "COINBULL.SILVER"
    CURRNOTE_CURRENCY = "CURRNOTE.CURRENCY"
    CURRNOTE_CURRYEAR = "CURRNOTE.CURRYEAR"
    # ESTV enum value, not a secret.
    CURRNOTE_TOKEN = "CURRNOTE.TOKEN"  # nosec: B105
    DEVT_COMBINEDPRODUCT = "DEVT.COMBINEDPRODUCT"
    DEVT_FUNDSIMILARASSET = "DEVT.FUNDSIMILARASSET"
    DEVT_INDEXBASKET = "DEVT.INDEXBASKET"
    FUND_ACCUMULATION = "FUND.ACCUMULATION"
    FUND_DISTRIBUTION = "FUND.DISTRIBUTION"
    FUND_REALESTATE = "FUND.REALESTATE"
    LIBOSWAP_LIBOR = "LIBOSWAP.LIBOR"
    LIBOSWAP_SWAP = "LIBOSWAP.SWAP"
    OPTION_CALL = "OPTION.CALL"
    OPTION_PHANTOM = "OPTION.PHANTOM"
    OPTION_PUT = "OPTION.PUT"
    SHARE_BEARERCERT = "SHARE.BEARERCERT"
    SHARE_BONUS = "SHARE.BONUS"
    SHARE_COMMON = "SHARE.COMMON"
    SHARE_COOP = "SHARE.COOP"
    SHARE_LIMITED = "SHARE.LIMITED"
    SHARE_LIMITEDOLD = "SHARE.LIMITEDOLD"
    SHARE_NOMINAL = "SHARE.NOMINAL"
    SHARE_PARTCERT = "SHARE.PARTCERT"
    SHARE_PREFERRED = "SHARE.PREFERRED"
    SHARE_TRANSFERABLE = "SHARE.TRANSFERABLE"
...
class LangText(PydanticXmlModel):
    ...
    email: Optional[
        Annotated[
            str, StringConstraints(pattern=r"[A-Za-z0-9._%+\-]+@[A-Za-z0-9._-]+\.[A-Za-z]{2,5}")
        ]
    ] = attr(default=None)
...
class Legend(Entity, tag="legend"):
    ...
    sign: Optional[Annotated[str, StringConstraints(min_length=3, max_length=4)]] = attr(
        default=None
    )
...
class YearendGrossNet(Yearend, tag="yearend"):
    ...
    ahvNumber: Optional[Annotated[str, StringConstraints(min_length=13, max_length=13)]] = attr(
        default=None
    )
...
class PaymentDerivative(Payment, tag="payment"):
    ...
    coupon: Optional[Annotated[str, StringConstraints(min_length=1, max_length=12)]] = attr(
        default=None
    )
...
class Security(Entity, tag="security"):  # Abstract base in XSD
    ...
    securityAppendix: Optional[Annotated[str, StringConstraints(min_length=1, max_length=60)]] = (
        attr(default=None)
    )
    sign: Optional[Annotated[str, StringConstraints(min_length=3, max_length=4)]] = attr(
        default=None
...
class Institution(Entity, tag="institution"):
    ...
    additionalPart: Optional[Annotated[str, StringConstraints(min_length=1, max_length=40)]] = attr(
        default=None
    )
    street: Optional[Annotated[str, StringConstraints(min_length=1, max_length=40)]] = attr(
        default=None
    )
    zip: Optional[Annotated[str, StringConstraints(min_length=1, max_length=12)]] = attr(
        default=None
    )
    city: Optional[Annotated[str, StringConstraints(min_length=1, max_length=40)]] = attr(
        default=None
    )
    canton: Optional[CantonBFS] = attr(default=None)
    country: CountryCode = attr(use="required")
    ...
    currency: Optional[CurrencyCode] = attr(default=None)
    domain: Optional[
        Annotated[str, StringConstraints(pattern=r"[A-Za-z0-9._-]+\.[A-Za-z]{2,4}")]
    ] = attr(default=None)
...
class Kursliste(PydanticXmlModel, tag="kursliste", nsmap=NSMAP):
    ...
    referingToDate: Optional[datetime.date] = attr(
        default=None
    )  # do NOT rename to referringToDate, aligned with XSD
    year: int = attr()
    # --- End Attributes ---
    ...
    securityGroups: List[DefinitionSecurityGroup] = element(
        tag="securityGroup", default_factory=list
    )
    ...
    capitalisationRates: List[CapitalisationRate] = element(
        tag="capitalisationRate", default_factory=list
    )
    ...
    exchangeRatesMonthly: List[ExchangeRateMonthly] = element(
        tag="exchangeRateMonthly", default_factory=list
    )
    exchangeRatesYearEnd: List[ExchangeRateYearEnd] = element(
        tag="exchangeRateYearEnd", default_factory=list
    )
    # --- End Elements ---
        ...
        if root.tag.startswith(f"{{{KURSLISTE_NS_2_0}}}"):
            ...
            def replace_ns(elem):
                ...
                if elem.tag.startswith(f"{{{KURSLISTE_NS_2_0}}}"):
                    elem.tag = elem.tag.replace(
                        f"{{{KURSLISTE_NS_2_0}}}", f"{{{KURSLISTE_NS_2_2}}}"
                    )
    ...
    @classmethod
    def from_xml_file(
        cls, file_path: Union[str, Path], denylist: Optional[Set[str]] = None
    ) -> "Kursliste":
        """
