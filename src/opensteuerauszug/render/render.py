...
class PageNumCanvas(canvas.Canvas):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._pageNumber = 1  # Initialize page number
    
    def showPage(self):
        self._saved_page_states.append(dict(self.__dict__))
        self._pageNumber += 1  # Move page number increment here
        canvas.Canvas.showPage(self)  # Call parent method directly
...
def generate_pdf(data, output_path=None):
    ...
    Args:
        data: Dictionary containing data for rendering
        output_path: Optional path to save the PDF to. If not provided, returns the PDF bytes.
        
    Returns:
        If output_path is None, returns the PDF bytes. Otherwise, writes to file and returns None.
    """
    buffer = io.BytesIO()
    ...
    # If output_path is provided, write the PDF to file
    if output_path:
        with open(output_path, 'wb') as f:
            f.write(pdf_data)
        return None
    
    # Otherwise, return the PDF bytes
    return pdf_data
...
def map_tax_statement_to_pdf_data(tax_statement: TaxStatement) -> Dict[str, Any]:
    ...
    if tax_statement.client and len(tax_statement.client) > 0:
        ...
        data["customer"] = {
            ...
            "firstName": client.firstName if hasattr(client, 'firstName') else "",
            "lastName": client.lastName if hasattr(client, 'lastName') else "",
            "address": "",  # Extract from address fields if available
            "city": "",     # Extract from address fields if available
            "zip": "",      # Extract from address fields if available
            "country": tax_statement.country if hasattr(tax_statement, 'country') else "CH"
        }
    ...
    data["period"] = {
        ...
        "from": tax_statement.periodFrom.isoformat() if hasattr(tax_statement, 'periodFrom') and tax_statement.periodFrom else "",
        "to": tax_statement.periodTo.isoformat() if hasattr(tax_statement, 'periodTo') and tax_statement.periodTo else ""
    }
