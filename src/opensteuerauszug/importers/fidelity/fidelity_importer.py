...
def should_skip_entry(entry: Any, entry_label: str) -> bool:
    ...
    account_id = (
        entry.get('Account Number') if (entry.get('Account Number')) else entry.get('Account')
    )
    ...
    if entry_label in ['Trade', 'Position']:
        ...
        if entry_label in ['Position']:
            ...
            if ending_value == 'unavailable':
                logger.debug(
                    "Skipping entry %s of type %s with no ending value.", entry, entry_label
                )
                return True
        if symbol is None or symbol == '':
            if (
                description is not None
            ):  # if there is no Description it's probably just an emptry line
                logger.info(
                ...
                logger.debug(
                    "Skipping entry %s of type %s with no description or symbol", entry, entry_label
                )
            return True
    if (account_id is None or account_id == '') and entry_label in ['Transactions']:
        logger.debug("Skipping entry %s of type %s with no Account Number", entry, entry_label)
        return True
    if symbol in SYMBOLS_TO_IGNORE or symbol == account_id:
        logger.debug("Skipping entry %s of type %s", symbol, entry_label)
        return True
    else:
        return False
...
class FidelityImporter:
    ...
    def _get_required_field(
        self, data_object: Any, field_name: str, object_description: str
    ) -> Any:
        ...
        if value is None or (
            ...
            and not value.strip()
            and not object_description == 'CashTransaction'
        ):
    ...
    def __init__(
        ...
        if not self.account_settings_list:
            ...
            logger.debug("FidelityImporter initialized with an empty list of " "account settings.")
        else:
            logger.debug("FidelityImporter initialized with settings %s", account_settings_list)
    ...
    def _read_statement(self, file_contents: list[str]):
        ...
        #    "Symbol/CUSIP,Description,Quantity,Price,Beginning Value,Ending Value,Cost Basis")
        position_data = list(
            csv.DictReader(file_contents[transaction_start_index:], skipinitialspace=True)
        )
        ...
        if len(position_data) and len(summary_data):
            logger.info(
                ...
                summary_data['Account Type'],
                summary_data['Account'],
            )
                    ...
                    if filename.find('Statement') > -1:
                        ...
                        try:
                            statement['Date'] = datetime.strptime(
                                filename.split('/')[-1].strip('.csv').strip('Statement'), '%m%d%Y'
                            ).date()
                        ...
                        transaction_data = self._read_transactions(
                            file_contents=csvfile.readlines()
                        )
    ...
    def _build_cash_transaction_security_position(
        ...
        return SecurityPosition(
            ...
            isin=None,
            symbol=str(symbol),
            description=(f"{description} ({symbol})" if symbol else description),
        )
    ...
    def import_files(self, filenames: List[str]) -> TaxStatement:
        ...
        if not all_statements:
            ...
            logger.warning(
                "No statements or were successfully parsed. " "Returning empty TaxStatement."
            )
            return TaxStatement(
                minorVersion=1,
        ...
        for stmt in all_statements:
            ...
            positions = self._get_required_field(stmt, 'Positions', 'Statement')
            ...
            for position in positions:
                ...
                symbol = self._get_required_field(position, 'Symbol/CUSIP', 'Position')
                if symbol.find('Stocks') == 0:
                    default_category = 'SHARE'
                    continue
                if (
                    len(symbol) == 0
                ):  # can happen with a delisted stock which will have an unknown value
                    logger.warning(
                ...
                if len(symbol) < 6:  # otherwise the line is something else
                    description = self._get_required_field(position, 'Description', 'Position')

                    quantity = self._to_decimal(
                        self._get_required_field(position, 'Quantity', 'Position'),
                        'position',
                        f"Position {symbol}",
                    )
                    ...
                    sec_pos = SecurityPosition(
                        depot=account_id, symbol=symbol, description=f"{description} ({symbol})"
                    )
                    ...
                    if sec_pos not in security_asset_category_map:
                        logger.debug(
                            f"Adding security position {sec_pos} asset category"
                            f" {asset_category}"
                        )
                        security_asset_category_map[sec_pos] = asset_category

                    mark_price = self._to_decimal(
                        position.get('Price'), 'Price', f"Position {symbol}"
                    )

                    pos_value = self._to_decimal(
                        position.get('Ending Value'), ' Ending Value', f"Position" f" {symbol}"
                    )
                    ...
                    raise NotImplementedError(
                        f"Fidelity Importer:Importing position {position} not implemented " f"yet."
                    )
        ...
        for transaction in all_transactions:
            ...
            account_id = ''
            symbol = None
            account_id = self._get_required_field(transaction, 'Account Number', 'Transactions')
            # account_id_processed = account_id # Keep track for summary
            action = None
            for key, value in ACTIONS_DICT.items():
                if transaction['Action'].find(key) > -1:
                    ...
                    break
                logger.debug(
                    "key:%s, value:%s, action:%s, found:%s ",
                    key,
                    value,
                    transaction['Action'],
                    transaction['Action'].find(key),
                )
            if action is None:
                raise ValueError(
                    f"Unknown {transaction['Action']} is not supported for {transaction['Description']}"
                )
            if not action:
                raise ValueError(f"Missing action in transaction: {transaction}")
            trade_date = self._get_required_field(transaction, 'Run Date', 'Transaction')
            # settle_date = transaction.get('Settlement Date') not usually present...

            description = self._get_required_field(transaction, 'Description', action)

            quantity = self._to_decimal(
                self._get_required_field(transaction, 'Quantity', action),
                'Quantity',
                f"Transaction {symbol if symbol is not None else action}",
            )
            # --- Process Trades ---
            ...
            if action in ['buy', 'sell', 'transfer', 'stock_split']:
                ...
                symbol = self._get_required_field(transaction, 'Symbol', action)
                ...
                asset_category = (
                    security_asset_category_map[sec_pos]
                    if sec_pos in security_asset_category_map
                    else 'SHARE'
                )
                # Added ETF, FUND
                if asset_category not in ["SHARE", "FUND"]:
                    asset_category = 'SHARE'
                    logger.warning(
                        f"Assuming asset category: {asset_category} " f"for (Symbol: {symbol})"
                    )
            ...
            # --- Process Cash Transactions ---
                ...
                amount = self._to_decimal(
                    self._get_required_field(transaction, 'Amount ($)', 'CashTransaction'),
                    'amount',
                ...
                if symbol:
                    ...
                    bank_payment = BankAccountPayment(
                        ...
                        name=transaction['Action'].replace(
                            "INTEREST EARNED", get_text("credit_interest", self.render_language)
                        ),
        ...
        for sec_pos_obj, data in processed_security_positions.items():
            ...
            asset_cat = (
                security_asset_category_map[sec_pos_obj]
            ...
            initial_reconciler = PositionReconciler(
                list(sorted_stocks), identifier=f"{sec_pos_obj.symbol}-initial_check"
            )
            ...
            reconciler = PositionReconciler(
                list(sorted_stocks),
                identifier=f"{sec_pos_obj.symbol}-reconcile",
            )
            ...
            trades_quantity_total = sum(s.quantity for s in sorted_stocks if s.mutation)
            ...
            start_exists = any(
                (not s.mutation and s.referenceDate == self.period_from) for s in sorted_stocks
            )
            ...
            end_exists = any(
                (not s.mutation and s.referenceDate == end_plus_one) for s in sorted_stocks
            )
            if not end_exists:
                sorted_stocks.append(
                    SecurityStock(
        ...
        if depot_securities_map:
            for depot_id, securities_in_depot in depot_securities_map.items():
                if securities_in_depot:
                    final_depots.append(
                        Depot(depotNumber=DepotNumber(depot_id), security=securities_in_depot)
                    )
        ...
        for stmt in all_statements:
            ...
            if summary.get('Ending Net Value') is not None:
                closing_net_value = self._to_decimal(
                    self._get_required_field(summary, 'Ending Net Value', 'Account Summary'),
                    'Ending Net Value',
            ...
            if summary.get('Ending mkt Value') is not None:
                closing_mkt_value = self._to_decimal(
                    self._get_required_field(summary, 'Ending mkt Value', 'Account Summary'),
                    'Ending mkt Value',
            ...
            if closing_balance_value is not None:
                if (
                    key not in all_currencies_with_balances.keys()
                    or end_date > all_currencies_with_balances[key]['date']
                ):
        ...
        list_of_bank_accounts = (
            ListOfBankAccounts(bankAccount=final_bank_accounts) if final_bank_accounts else None
        )
        ...
        if not account_id:
            logger.warning('Account ID not found in first statement: %s', summary)
        ...
        canton_raw = (
            getattr(matching_settings, 'canton', None) if matching_settings is not None else None
        )
        ...
        if canton:
            ...
            logger.warning("Invalid canton in account settings: %r", canton_raw)
...
def main():
    ...
    parser.add_argument(
        '--config_file', type=str, default='config.toml', help="config file path", required=False
    )
    parser.add_argument(
        '-strict_consistency',
        action='store_true',
        help='Enable/disable strict consistency ' 'checks in importers. Defaults to strict.',
        required=False,
    )
    ...
    concrete_accounts_list = config_manager.get_all_account_settings_for_broker("fidelity")
