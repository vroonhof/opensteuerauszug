...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="local-action-") as temp_dir:
        ...
        if details_url:
            local_summary_lines[2] = f"Artifacts: {details_url}"
        if command_result.stderr:
            local_summary_lines.append("")
            local_summary_lines.append("Stderr:")
            local_summary_lines.append(command_result.stderr.rstrip())
        else:
            local_summary_lines.append("")
            local_summary_lines.append("Stderr: (none)")
        if args.print_check_summary:
            print("--- check summary ---")
            print("\n".join(local_summary_lines))

        if args.upload_artifacts:
            upload_summary_lines = list(local_summary_lines)
        else:
            if command_result.stderr:
                upload_summary_lines = [command_result.stderr.rstrip()]
            else:
                upload_summary_lines = ["Stderr: (none)"]
