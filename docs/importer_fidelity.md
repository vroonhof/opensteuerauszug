...
* OpenSteuerAuszug is a tool to assist you in preparing the Steuerauszug, but it does not absolve you of your legal obligations. Any errors or omissions in your tax filing are your responsibility.
...
All data files should be downloaded in a single directory. The software will detect the file types automatically. The user provides the directory on the commandline.
...
The Fidelity importer **does not** calculate the end of period cash values in your account, this is taken directly from the statement with the latest date in the title.

## Equity Awards
Currently not-supported

# Defined Actions
The following 'actions' are identified from the Description string and should be handled correctly:
...
## Required Inputs
One will need to download data for your accounts separately. 
...
*   **How to obtain**:
    ...
    1. From there download monthly statements in CSV format.
...
In principle, one only needs the statement for the last month of your tax period.  This is where the importer will get positions, and the cash value in the account from. The annual statement is only available as a PDF and is not used. 
...
Fidelity doesn't contain ones name or address in the exported files. So one should provide their name and canton.
...
Use `config.toml` to configure one's Fidelity accounts. There is a subsection for each account, the name of the section does not matter.:
...
One needs to provide a (reusable) mapping in `data/security_identifiers.csv`.
...
*   **Missing symbols**:  for some transaction the symbol is not listed in the correct column, but is listed in the Description. In this case it is best to edit the csv by hand.
*   **Incorrect dates**:  For some items to match the kursliste date we need is in the description (something handled correctly by the importer), but watch out for this!
## Troubleshooting
